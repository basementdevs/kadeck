...
import type { DeckDevice } from '@/vite-env';
...
export default function StreamDeckPlus({ droppedItems }: DeckDevicesProps) {
  ...
  const streamDeckButtonsDropzone = Array.from({ length: streamDeckItems.buttons }, (_, index) =>
    generateDropzoneIndexKey(streamDeckItems, 'Button', index)
  );
  const streamDeckScreensDropzone = Array.from({ length: streamDeckItems.screens }, (_, index) =>
    generateDropzoneIndexKey(streamDeckItems, 'Screen', index)
  );
  const streamDeckKnobsDropzone = Array.from({ length: streamDeckItems.knobs }, (_, index) =>
    generateDropzoneIndexKey(streamDeckItems, 'Knob', index)
  );
  return (
