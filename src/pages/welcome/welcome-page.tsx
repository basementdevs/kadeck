...
import {Github, FileDown, PlusCircle, Trash2, FolderOpen, Gamepad, Gamepad2} from "lucide-react"
...
export function WelcomePage() {
  ...
  const changeDefaultProfile = (profileId: string) => {
    // @ts-ignore
    invoke("set_default_profile", { profileId: profileId }).then(() => {
        console.log("Profile changed")
        fetchProfiles()
    }).catch((e) => {
        alert("Error changing profile" + e)
    })
  }
        ...
        <CardContent className="flex flex-col items-center space-y-4 p-6">
          ...
          >
            ...
            <Card className="bg-gray-800/50 border-none shadow-inner">
              <CardHeader>
                <CardTitle className="text-white text-center text-xl">Load Profile ({profileManager.profiles.length })</CardTitle>
              </CardHeader>
              <CardContent>
                <ScrollArea className={`w-full pr-4 ${profileManager.profiles.length === 0 ? 'h-0' : 'h-[250px]'}`}>
                  {profileManager.profiles.map((profile) => (
                      ...
                      >
                        <div className={`flex flex-col rounded px-2 ${profile.id == profileManager.selected_profile ? "border-blue-500 border-l-4 " : ""} `}>
                          <span className="font-semibold text-white">{profile.name}</span>
                          <span className="text-sm text-gray-400">Created: {profile.created_at.toDateString()}</span>
                        </div>
